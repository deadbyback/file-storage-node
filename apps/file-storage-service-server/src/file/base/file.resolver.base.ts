...
export class FileResolverBase {
  ...
  async createFile(@graphql.Args() args: CreateFileArgs): Promise<File> {
    return await this.service.createFile({
      ...
      data: {
        ...
        archive: args.data.archive
          ? {
              connect: args.data.archive,
            }
          : undefined,
      },
    });
  ...
  async getArchive(@graphql.Parent() parent: File): Promise<Archive | null> {
    const result = await this.service.getArchive(parent.id);
  ...
  async DeleteFile(
    ...
    return this.service.DeleteFile(args);
  }
}
